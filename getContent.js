 ...
 *************************************************************************/
 function getContent(user_input,callback){
	...
	var sqlQuery = 'select * from place_desc where place_name= ? and place_city= ?';
	...
	connection.query(sqlQuery,sqlQueryParams,function(err,rows,fields){
		if(err){
			...
			async.parallelLimit([
				...
				async.parallel([
				  ...
				  // 		console.log('图片链接：'+picUrl);
				        ...
				        dataRes["rank"] = rating;
				        dataRes["engName"] = enName;
				        dataRes["picUrl"] = picUrl;

				        //console.log(jsonRes);
				        callback(jsonRes);

		        		var connectionInsert=DB.Connect();
						var sqlInsert = 'Insert Into place_desc(place_name,place_city,keywords,description,ticket,price,openTime,bestTime,ranking,engName,picUrl) VALUES(?,?,?,?,?,?,?,?,?,?,?)';
						var sqlInsertParams = [placeName,placeCity,keywordString,description,ticket,price,openTime,bestTime,rating,enName,picUrl];
			...
			var sqlUpdate = 'update place_desc set hot=hot+1 where place_name= ? and place_city = ?';
	        ...
	        dataRes["rank"] = rows[0]["ranking"];
	        dataRes["engName"] = rows[0]["engName"];
	        dataRes["picUrl"] = rows[0]["picUrl"];
			//console.log(jsonRes);
			callback(jsonRes);
			//return;
		}
